...
The following is the code used to set the working directory and load the data set and libraries used.
...
We would assume that for any given day, activity level wouldn't differ significantly. But, depending on the persons line of work, cold he be more active during the week or on the weekend? This could give us some insight into the person. For, if he is more active on the weekend we could conclude that maybe he has a sedentary career, while if the contrary is true maybe the person has a very active job and prefers to relax during the weekend.

The following panel plot gives a time series of the average number of steps across an interval, as with the previous time series, but splits activity day by "weekday" and "weekend", to give us a side by side visual representation.
...
###Conclusions
The graphs indicates that activity behaviour doesn't differ significantly with regard to the time of week. Even considering the effects of missing values on the data set, we can observe that it is nearly equally as likely that they are missing on the weekend as during the week. So, we can reasonably conclude that activity behaviour is constant during the week and that if the person's job is sedentary so is his leisure time and vice versa (that or the individual works 7 days a week).
