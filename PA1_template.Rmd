...
We're missing `r total_missing` observations out of a total number of `r total_observed`, which means that approximately `as.integer((1 - total_missing / total_observed) * 100)`% of interval observations were not missing.
...
```{r, echo=FALSE}
activity_new[is.na(activity_new$steps) & activity_new$interval==names(avg_steps_overint)[j], ]
```
...
ts_mean_new = mean(total_steps_new)
ts_median_new = median(total_steps_new)
hist(total_steps_new, main =paste0("Histogram of total number of steps taken per day \n (mean = ", as.integer(ts_mean_new), ", median = ", ts_median_new,")"), xlab = "Steps", ylab = "Days")
```
